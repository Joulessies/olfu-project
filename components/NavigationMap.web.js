/**
 * Navigation Map Component - WEB VERSION
 * Uses iframe with Leaflet for web browsers
 * 
 * Note: Native version is in NavigationMap.native.js
 */
...
const NavigationMap = ({
  ...
  const startLocationTracking = async () => {
    try {
      ...
      // Fetch route info
      if (destination) {
        fetchRouteInfo(location.coords.latitude, location.coords.longitude);
      }
  ...
  const fetchRouteInfo = async (userLat, userLng) => {
    try {
      const response = await fetch(
        `https://router.project-osrm.org/route/v1/driving/${userLng},${userLat};${destination.longitude},${destination.latitude}?overview=full&geometries=geojson&steps=true`
      );
      ...
      if (data.code === "Ok" && data.routes?.length > 0) {
        ...
        if (route.legs[0].steps.length > 0) {
          setCurrentStep({
            ...
            distance: route.legs[0].steps[0].distance,
          });
        }
      }
    } catch (error) {
  ...
  const generateNavigationHTML = () => {
    if (!userLocation || !destination) return "<html><body style='display:flex;justify-content:center;align-items:center;height:100%;'>Loading map...</body></html>";
    ...
    return `<!DOCTYPE html><html><head>
      ...
      var map = L.map('map',{zoomControl:true}).setView([${userLat},${userLng}],15);
      L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);
      
      // User marker
      L.marker([${userLat},${userLng}],{icon:L.divIcon({className:'',html:'<div style="width:20px;height:20px;background:#2196F3;border-radius:50%;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);"></div>',iconSize:[20,20],iconAnchor:[10,10]})}).addTo(map).bindPopup('Your Location');
      
      // Destination marker
      L.marker([${destLat},${destLng}],{icon:L.divIcon({className:'',html:'<div style="background:#D32F2F;width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);"><span style="font-size:14px;">🏫</span></div>',iconSize:[30,30],iconAnchor:[15,15]})}).addTo(map).bindPopup('${destinationName}');
    ...
    <View style={[styles.container, style]}>
      ...
      {currentStep && (
        <Animated.View style={[styles.instructionPanel, { transform: [{ scale: pulseAnim }] }]}>
          <View style={styles.turnIconContainer}>
            <Ionicons name={getTurnIcon(currentStep.instruction)} size={32} color={Colors.textOnPrimary} />
          </View>
      ...
      <View style={styles.bottomBar}>
        <View style={styles.osmBadge}>
          ...
          <Text style={styles.osmBadgeText}>OpenStreetMap • Free Navigation</Text>
        </View>
        <View style={styles.bottomInfo}>
          <View style={styles.infoItem}>
            <Ionicons name="time-outline" size={20} color={Colors.primary} />
            <Text style={styles.infoValue}>{routeInfo?.duration || "--"} min</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  mapContainer: { flex: 1, position: "relative", minHeight: 300 },
